...
class Conv1dGenerated(torch.nn.Module):
    ...
    def __init__(
        ...
        self._bottleneck = nn.Linear(embedding_dim, bottleneck_dim)
        self._kernel = nn.Linear(
            bottleneck_dim, out_channels // groups * in_channels // groups * kernel_size
        )
        self._bias = nn.Linear(bottleneck_dim, out_channels // groups) if bias else None
    ...
    def forward(self, generator_embedding, x):
        ...
        kernel = self._kernel(e).view(
            self._out_channels, self._in_channels // self._groups, self._kernel_size
        )
        bias = self._bias(e).view(self._out_channels) if self._bias else None

        return F.conv1d(
            x, kernel, bias, self._stride, self._padding, self._dilation, self._groups
        )
